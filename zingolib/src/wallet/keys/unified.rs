...
use zcash_transparent::keys::{
    AccountPrivKey, AccountPubKey, IncomingViewingKey, NonHardenedChildIndex,
};
...
impl TryFrom<&UnifiedKeyStore> for AccountPrivKey {
    type Error = KeyError;
    fn try_from(unified_key_store: &UnifiedKeyStore) -> Result<Self, Self::Error> {
        let usk = UnifiedSpendingKey::try_from(unified_key_store)?;
...
impl TryFrom<&UnifiedKeyStore> for AccountPubKey {
    type Error = KeyError;
    fn try_from(unified_key_store: &UnifiedKeyStore) -> Result<Self, Self::Error> {
        let ufvk = UnifiedFullViewingKey::try_from(unified_key_store)?;
        ufvk.transparent()
